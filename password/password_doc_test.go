...
func ExampleGenerate() {
	res, err := password.Generate(64, 10, 10, 26, false)
	if err != nil {
		log.Fatal(err)
	}
	log.Print(res)
...
func ExampleMustGenerate() {
	// Will panic on error
	res := password.MustGenerate(64, 10, 10, 26, false)
	log.Print(res)
}
...
func ExampleGenerator_Generate() {
	...
	res, err := gen.Generate(64, 10, 10, 26, false)
	if err != nil {
		log.Fatal(err)
	}
	log.Print(res)
...
func ExampleNewMockGenerator_testing() {
	...
	f := func(g password.PasswordGenerator) string {
		...
		return g.MustGenerate(1, 2, 3, 4, false)
	}
